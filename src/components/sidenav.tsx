              ...
              {tags.map((tag) => (
                <a key={tag.name} href="#">
                  <span
                    className={classNames(
                      tag.color
                        ? `bg-${tag.color}-100 text-${tag.color}-900 dark:bg-${tag.color}-900 dark:text-${tag.color}-100`
                        : "bg-gray-200 text-gray-800 dark:bg-gray-700 dark:text-gray-200",
                      "inline-flex items-center rounded-full px-3 py-0.5 text-sm font-medium hover:underline m-0.5"
                    )}
                  >
                    {tag.name}{" "}
                    <span className="bg-gray-950/5 dark:bg-gray-500/20 rounded-md px-1 ml-1 text-xs">
                      {tag.count}
                    </span>
                  </span>{" "}
                </a>
